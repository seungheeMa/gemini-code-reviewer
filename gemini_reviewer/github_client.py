...
from .models import PRDetails, ReviewComment, ReviewResult, ReviewPriority
...
class GitHubClient:
    ...
    def _validate_and_sanitize_comment(self, comment: ReviewComment) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Add line number to comment body for better context
            enhanced_body = f"**라인 {comment.line_number}:** {comment.body}"
            
            # Add priority indicator if high priority
            if comment.priority in [ReviewPriority.HIGH, ReviewPriority.CRITICAL]:
                priority_emoji = "🚨" if comment.priority == ReviewPriority.CRITICAL else "⚠️"
                enhanced_body = f"{priority_emoji} **{comment.priority.value.upper()}**\n{enhanced_body}"
            
            # Sanitize content
            sanitized_comment = {
                'body': self._sanitize_input(enhanced_body),
    ...
    def _generate_review_summary(self, comments: List[ReviewComment]) -> str:
        ...
        summary_parts = ["🤖 **Gemini AI 코드 리뷰**"]
        summary_parts.append(f"\n**{len(comments)}**개의 개선 제안을 찾았습니다:")
        
        priority_korean = {"critical": "치명적", "high": "높음", "medium": "보통", "low": "낮음"}
        for priority, count in priority_counts.items():
            emoji = {"critical": "🚨", "high": "⚠️", "medium": "💡", "low": "ℹ️"}.get(priority, "📝")
            korean_priority = priority_korean.get(priority, priority.title())
            summary_parts.append(f"- {emoji} {korean_priority}: {count}")
        
        summary_parts.append(f"\n> 이 리뷰는 Gemini AI에 의해 자동으로 생성되었습니다. 제안사항을 신중하게 검토해 주세요.")
