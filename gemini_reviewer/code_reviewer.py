...
class CodeReviewer:
    ...
    async def review_pull_request(self, event_path: str) -> ReviewResult:
        ...
        logger.info("=== Pull Request 리뷰 시작 ===")
        ...
        try:
            ...
            if comments:
                ...
                logger.info("생성된 코멘트 없음 - 코드가 좋아 보입니다! 👍")
    ...
    def _convert_to_review_comment(
        ...
        try:
            ...
            # GitHub API uses position within the diff, not absolute line numbers
            position = self._calculate_diff_position(ai_response.line_number, hunk)
            
            # Ensure position is valid
            if position <= 0:
                logger.warning(f"Invalid position {position} calculated for line {ai_response.line_number}")
                return None
    ...
    def _calculate_diff_position(self, target_line: int, hunk: HunkInfo) -> int:
        """Calculate the correct position in the diff for GitHub API."""
        try:
            # GitHub position is the line number within the hunk where the comment should be placed
            # We need to find the line in the hunk that corresponds to the target line
            
            current_line = hunk.target_start
            position_in_hunk = 1  # GitHub positions are 1-based within the hunk
            ...
            for line in hunk.lines:
                if line.startswith(' '):  # Context line
                    ...
                    current_line += 1
                    if current_line - 1 == target_line:  # Check if this is our target line
                        return position_in_hunk
                    position_in_hunk += 1
                elif line.startswith('-'):  # Removed line - doesn't affect target line numbers
                    continue
                
                # Safety check - don't go too far
                if position_in_hunk > len(hunk.lines) + 10:
                    break
            
            # Fallback: if we can't find the exact line, use a reasonable position
            # Place comment at the first added line or at the end of hunk
            for i, line in enumerate(hunk.lines):
                if line.startswith('+'):
                    return i + 1  # Return position of first added line
            
            # Last resort - place at the end of hunk
            return len(hunk.lines)
            
        except Exception as e:
            logger.warning(f"Error calculating diff position: {str(e)}")
            # Safe fallback
            return min(target_line, len(hunk.lines))
